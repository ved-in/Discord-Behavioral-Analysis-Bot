...
def generate_chart(metrics, username, output_dir=None):
    ...
    draw_radar(ax, radar_values(metrics), "#7B68EE", dot_colors=[DIMENSION_COLORS[l] for l in LABELS])
    ax.set_title(f"Behavioral Profile — {username}", color="white", fontsize=14, fontweight="bold", pad=20)

    score_text = "  |  ".join(
        f"{label}: {metrics[attr]:.0f}" for label, attr in RADAR_DIMENSIONS
    )
    fig.text(0.5, 0.02, score_text, ha="center", va="bottom", color="#aaaacc", fontsize=8, fontfamily="monospace")
    return save_chart(fig, "radar", output_dir)


def generate_comparison_chart(metrics1, name1, metrics2, name2, output_dir=None):
    output_dir = output_dir or tempfile.gettempdir()
    fig, axes = plt.subplots(1, 2, figsize=(16, 8), subplot_kw={"polar": True}, facecolor="#1a1a2e")
    for ax, metrics, name, color in zip(axes, [metrics1, metrics2], [name1, name2], ["#7B68EE", "#FF6B6B"]):
